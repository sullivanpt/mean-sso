...
/**
 * Helper to test if at least one of the desired scopes have been granted
 ...
 * @param possessed
 */
function checkScope(desired, possessed) {
  if (possessed.indexOf('*') !== -1) {
    return true;
  }
  if(Array.isArray(desired)) {
    for (var i = desired.length; i--; ){
      if((possessed || []).indexOf(desired[i]) !== -1) {
        return true;
      }
    }
  } else {
    if((possessed || []).indexOf(desired) !== -1) {
      return true;
    }
  }
  return false;
...
function hasAuthorization(options) {
  ...
  return function(req, res, next) {
    if (!options.anonymous) {
      if (options.role && !req.user.hasRole(options.role)) {
        return res.send(403, 'Forbidden');
      }
      if (options.groups && !req.user.hasGroup(options.groups)) {
        return res.send(403, 'Forbidden');
      }
      if (options.scope && req.authInfo && !checkScope(options.scope, req.authInfo.scope)) {
        return res.send(403, 'Forbidden');
      }
    }
    next();
