...
/**
 * Handles requests to obtain a grant in the form of a CAS ticket.
 *
 * Copied from: mean-sso\node_modules\oauth2orize\lib\grant\code.js
 *
 * Callbacks:
 *
...
module.exports = function code(options, issue) {
    ...
    if (!issue) { throw new TypeError('oauth2orize.cas grant requires an issue callback'); }
    ...
    /* Parse requests that request `cas` as `response_type`.
     *
     * @param {http.ServerRequest} req
     * @api public
     */
    function request(req) {
        var redirectURI = req.query.service
            , scope = ['login']
            , state = req.query.state;

        if (!redirectURI) { throw new AuthorizationError('Missing required parameter: service', 'invalid_request'); }
    ...
    /* Sends responses to transactions that request `cas` as `response_type`.
     *
     * @param {Object} txn
    ...
    function response(txn, res, next) {
        ...
        function issued(err, code) {
            ...
            var parsed = url.parse(txn.redirectURI, true);
            delete parsed.search;
            parsed.query.ticket = 'ST-' + code; // ticket prefix is required
            if (txn.req && txn.req.state) { parsed.query.state = txn.req.state; }
