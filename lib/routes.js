...
var api = require('./controllers/api'),
  ...
  personae = require('./controllers/personae'),
  socialMessages = require('./controllers/social-messages'),
...
module.exports = function(app) {
  ...
  // for hiding sensitive persona details we use entirely different resource routing (as opposed to caller context)

  app.options('/api2/me/personae', securityPolicy.corsPreflight);
  app.options('/api2/me/personae/*', securityPolicy.corsPreflight);
  app.post('/api2/me/personae', securityPolicy.enforce('knownUserApi'), personae.create);
  app.get('/api2/me/personae', securityPolicy.enforce('knownUserApi'), personae.list);
  app.get('/api2/me/personae/:personaID', securityPolicy.enforce('knownUserApi'), personae.show);
  app.put('/api2/me/personae/:personaID', securityPolicy.enforce('knownUserApi'), personae.update);
  app.delete('/api2/me/personae/:personaID', securityPolicy.enforce('knownUserApi'), personae.delete);
  app.options('/api2/personae', securityPolicy.corsPreflight);
  app.options('/api2/personae/*', securityPolicy.corsPreflight);
  app.get('/api2/personae/:personaID', securityPolicy.enforce('knownUserApi'), personae.showPublic);
  app.get('/api2/personae', securityPolicy.enforce('knownUserApi'), personae.searchPublic);

  app.options('/api2/messages', securityPolicy.corsPreflight);
  app.options('/api2/messages/*', securityPolicy.corsPreflight);
  app.post('/api2/messages', securityPolicy.enforce('knownUserApi'), socialMessages.send);
