...
module.exports = function(app) {
  ...
  app.post('/api/users', middleware.apiEnsureAuthorization({ anonymous: true, limit: 'byAnyone' }), users.create);
  app.put('/api/users', users.changePassword);
  app.get('/api/users/me', users.me);
  app.get('/api/users/:id', users.show);

  app.post('/api/session', middleware.apiEnsureAuthorization({ anonymous: true, authLimit: 'byAnyone' }), session.login);
  ...
  app.post('/oauth2/token', oauth2.token); // TODO: apply middleware.apiEnsureAuthorization({ anonymous: true, authLimit: 'byAnyone' }) here and similarly
